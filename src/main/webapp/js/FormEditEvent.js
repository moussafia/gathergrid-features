...
let optionsEdit = [];
$("#type_ticket_updated option").each(function(index,value){
    let optionObject = {
        value : index,
        isSelected : false,
        typeTicket : $(value).text()
    }
    optionsEdit.push(optionObject);
})
...
$(document).on("click",".edit-event-btn",function (){
    ...
    dataTicketJson.forEach((value, index)=>{
        appendInputSelect(value.ticketQuantity, value.ticketType, value.ticketPrice, index)
    })

})
function appendInputSelect(quntity, typeTicket, ticketPrice, index){
    if($("#show_item-updated")[0].childElementCount < 4 && index > 0){
        appendFormTicket()
    }
    $(".quantity-edit:eq("+index+")").val(quntity);
    $(".type_ticket_updated:eq("+index+")").val(typeTicket)
    $(".price-edit:eq("+index+")").val(ticketPrice);
}
